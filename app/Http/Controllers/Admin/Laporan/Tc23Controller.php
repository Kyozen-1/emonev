...
use Excel;
use PDF;
use App\Exports\Tc23Ekspor;

class Tc23Controller extends Controller
{
    public function tc_23(Request $request)
    {
        ...
        return response()->json(['tc_23' => $tc_23]);
    }

    public function tc_23_ekspor_pdf($opd_id)
    {
        $get_periode = TahunPeriode::where('status', 'Aktif')->latest()->first();
        $tahun_awal = $get_periode->tahun_awal;
        $tahun_akhir = $get_periode->tahun_akhir;
        ...
        $pdf = PDF::loadView('opd.laporan.tc-23', [
            'tc_23' => $tc_23,
        ]);

        return $pdf->setPaper('b2', 'landscape')->stream('Laporan TC - 23.pdf');
    }

    public function tc_23_ekspor_excel($opd_id)
    {
        return Excel::download(new Tc23Ekspor($opd_id), 'Laporan TC-23.xlsx');
    }
}
