...
use Excel;
use PDF;
use App\Exports\OpdTc24Ekspor;

class Tc24Controller extends Controller
{
    public function tc_24()
    {
        $tc_24 = '';
        $tws = MasterTw::all();
                            ...
                            foreach($kegiatans as $kegiatan)
                            {
                                ...
                                foreach ($sub_kegiatan_indikator_kinerjas as $sub_kegiatan_indikator_kinerja) {
                                    $tc_24 .= '<tr>';
                                        ...
                                        // Kolom 2 - 6 Start
                                        foreach ($tahuns as $tahun) {
                                            $sub_kegiatan_target_satuan_rp_realisasi = SubKegiatanTargetSatuanRpRealisasi::whereHas('opd_sub_kegiatan_indikator_kinerja', function($q) use ($sub_kegiatan_indikator_kinerja){
                                                                                            $q->where('opd_id', Auth::user()->opd->opd_id);
        ...
        return response()->json(['tc_24'=> $tc_24]);
    }

    public function tc_24_ekspor_excel()
    {
        return Excel::download(new OpdTc24Ekspor, 'Laporan TC-24.xlsx');
    }

    public function tc_24_ekspor_pdf()
    {
        $tc_24 = '';
        $tws = MasterTw::all();
        $get_periode = TahunPeriode::where('status', 'Aktif')->latest()->first();
        ...
        $pdf = PDF::loadView('opd.laporan.tc-24', [
            'tc_24' => $tc_24,
        ]);

        return $pdf->setPaper('b2', 'landscape')->stream('Laporan TC - 24.pdf');
    }
}
