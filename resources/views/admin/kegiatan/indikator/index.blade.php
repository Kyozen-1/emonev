...
@section('title', 'Admin | Kegiatan | Indikator')
@section('css')
...
@section('content')
    ...
    <div class="container">
        ...
        <div class="card mb-5">
            <div class="card-body">
                @php
                    $cek_perubahan_program = PivotPerubahanProgram::where('program_id', $kegiatan->program_id)->latest()->first();
                ...
                <p>
                    ...
                    <br>
                    Kode Kegiatan: {{$kegiatan->kode}}<br>
                    Kegiatan: {{$kegiatan->deskripsi}}<br>
                </p>
            </div>
        </div>
        ...
        <div class="data-table-rows slim">
            ...
            <div class="data-table-responsive-wrapper">
                <table id="kegiatan_indikator_table" class="data-table w-100">
                    <thead>
                        <tr>
        ...
        <div class="modal-dialog modal-dialog-centered modal-lg">
            <div class="modal-content">
                ...
                <div class="modal-body">
                    ...
                    <form id="kegiatan_indikator_form" class="tooltip-label-end" method="POST" novalidate enctype="multipart/form-data">
                        @csrf
                        <input type="hidden" name="kegiatan_id" value="{{$kegiatan->kegiatan_id?$kegiatan->kegiatan_id:$kegiatan->id}}">
                        <div class="row">
                            <div class="mb-3">
    ...
    <div id="importModal" class="modal fade" tabindex="-1" role="dialog" aria-labelledby="importModalLabel" aria-hidden="true">
        <div class="modal-dialog modal-dialog-centered modal-lg">
            <div class="modal-content">
                ...
                <div class="modal-body">
                    <form action="{{ route('admin.kegiatan.indikator.impor') }}" class="form-horizontal" method="POST" enctype="multipart/form-data">
                        @csrf
                        <input type="hidden" name="kegiatan_id" value="{{$kegiatan->kegiatan_id?$kegiatan->kegiatan_id:$kegiatan->id}}">
                        <div class="mb-3 position-relative form-group">
                            <input type="file" class="dropify" id="impor_kegiatan_indikator" name="impor_kegiatan_indikator" data-height="150" data-allowed-file-extensions="xlsx" data-show-errors="true" required>
                        </div>
                        <div class="mb-3 position-relative form-group">
...
<script src="{{ asset('acorn/acorn-elearning-portal/js/vendor/bootstrap-submenu.js') }}"></script>
    ...
    <script>
        var kegiatan_id = "{{$kegiatan->kegiatan_id?$kegiatan->kegiatan_id:$kegiatan->id}}";
        $(document).ready(function(){
            $('.dropify').dropify();
            $('.dropify-wrapper').css('line-height', '3rem');

            var dataTables = $('#kegiatan_indikator_table').DataTable({
                processing: true,
                serverSide: true,
                ajax: {
                    url: "{{ url('/admin/kegiatan/"+kegiatan_id+"/indikator') }}",
                    data: {
                        id : kegiatan_id
                    }
                },
        ...
        $(document).on('click', '.detail', function(){
            var id = $(this).attr('id');
            $.ajax({
                url: "{{ url('/admin/kegiatan/indikator/detail') }}"+'/'+id,
                dataType: "json",
        ...
        $('#create').click(function(){
            $('#kegiatan_indikator_form')[0].reset();
        ...
        $('#kegiatan_indikator_form').on('submit', function(e){
            e.preventDefault();
            if($('#aksi').val() == 'Save')
            {
                $.ajax({
                    url: "{{ url('/admin/kegiatan/indikator') }}",
                    method: "POST",
                    data: $(this).serialize(),
                    dataType: "json",
                    beforeSend: function()
                    ...
                    success: function(data)
                    {
                        ...
                        if(data.errors)
                        {
                            ...
                            $('#kegiatan_indikator_form')[0].reset();
                            $('#aksi_button').text('Save');
                            $('#kegiatan_indikator_table').DataTable().ajax.reload();
                        }
                        if(data.success)
                        {
            ...
            {
                $.ajax({
                    url: "{{ url('/admin/kegiatan/indikator/update') }}",
                    method: "POST",
                    data: $(this).serialize(),
                    dataType: "json",
                    beforeSend: function(){
        ...
        $(document).on('click', '.edit', function(){
            ...
            $('#form_result').html('');
            $.ajax({
                url: "{{ url('/admin/kegiatan/indikator/edit') }}"+'/'+id,
                dataType: "json",
                success: function(data)
                {
        ...
        $(document).on('click', '.delete',function(){
            ...
            return new swal({
                ...
                {
                    $.ajax({
                        url: "{{ url('/admin/kegiatan/indikator/destroy') }}" + '/' + id,
                        dataType: "json",
                        beforeSend: function()
                        {
                            return new swal({
