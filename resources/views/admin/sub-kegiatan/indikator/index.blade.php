...
@section('title', 'Admin | Sub Kegiatan | Indikator')
@section('css')
...
@section('content')
    ...
    <div class="container">
        ...
        <div class="card mb-5">
            <div class="card-body">
                @php
                    $cek_perubahan_kegiatan = PivotPerubahanKegiatan::where('kegiatan_id', $sub_kegiatan->kegiatan_id)->latest()->first();
                    if($cek_perubahan_kegiatan)
                    {
                        ...
                        $kegiatan = Kegiatan::find($sub_kegiatan->kegiatan_id);
                ...
                <p>
                    ...
                    Urusan : {{$deskripsi_urusan}} <br>
                    <br>
                    Kode Program: {{$kode_program}}<br>
                    Program: {{$deskripsi_program}}<br>
                    <br>
                    Kode Kegiatan: {{$kode_kegiatan}}<br>
                    Kegiatan: {{$deskripsi_kegiatan}}<br>
                    <br>
                    Kode Sub Kegiatan: {{$sub_kegiatan->kode}}<br>
                    Sub Kegiatan: {{$sub_kegiatan->deskripsi}}<br>
                </p>
            </div>
        </div>
        ...
        <div class="row mb-3">
            <div class="col-12" style="text-align: right">
                ...
                <a class="btn btn-outline-success waves-effect waves-light mr-2" href="{{ asset('template/template_impor_sub_kegiatan_indikator.xlsx') }}" title="Download Template Import Data"><i class="fas fa-file-excel"></i></a>
        ...
        <div class="data-table-rows slim">
            ...
            <div class="data-table-responsive-wrapper">
                <table id="sub_kegiatan_indikator_table" class="data-table w-100">
                    <thead>
                        <tr>
        ...
        <div class="modal-dialog modal-dialog-centered modal-lg">
            <div class="modal-content">
                ...
                <div class="modal-body">
                    ...
                    <form id="sub_kegiatan_indikator_form" class="tooltip-label-end" method="POST" novalidate enctype="multipart/form-data">
                        @csrf
                        <input type="hidden" name="sub_kegiatan_id" value="{{$sub_kegiatan->sub_kegiatan_id?$sub_kegiatan->sub_kegiatan_id:$sub_kegiatan->id}}">
                        <div class="row">
                            <div class="mb-3">
    ...
    <div id="importModal" class="modal fade" tabindex="-1" role="dialog" aria-labelledby="importModalLabel" aria-hidden="true">
        <div class="modal-dialog modal-dialog-centered modal-lg">
            <div class="modal-content">
                ...
                <div class="modal-body">
                    <form action="{{ route('admin.sub-kegiatan.indikator.impor') }}" class="form-horizontal" method="POST" enctype="multipart/form-data">
                        @csrf
                        <input type="hidden" name="sub_kegiatan_id" value="{{$sub_kegiatan->sub_kegiatan_id?$sub_kegiatan->sub_kegiatan_id:$sub_kegiatan->id}}">
                        <div class="mb-3 position-relative form-group">
                            <input type="file" class="dropify" id="impor_sub_kegiatan_indikator" name="impor_sub_kegiatan_indikator" data-height="150" data-allowed-file-extensions="xlsx" data-show-errors="true" required>
                        </div>
                        <div class="mb-3 position-relative form-group">
...
<script src="{{ asset('acorn/acorn-elearning-portal/js/vendor/bootstrap-submenu.js') }}"></script>
    ...
    <script>
        var sub_kegiatan_id = "{{$sub_kegiatan->sub_kegiatan_id?$sub_kegiatan->sub_kegiatan_id:$sub_kegiatan->id}}";
        $(document).ready(function(){
            $('.dropify').dropify();
            $('.dropify-wrapper').css('line-height', '3rem');

            var dataTables = $('#sub_kegiatan_indikator_table').DataTable({
                processing: true,
                serverSide: true,
                ajax: {
                    url: "{{ url('/admin/sub-kegiatan/"+sub_kegiatan_id+"/indikator') }}",
                    data: {
                        id : sub_kegiatan_id
                    }
                },
        ...
        $(document).on('click', '.detail', function(){
            var id = $(this).attr('id');
            $.ajax({
                url: "{{ url('/admin/sub-kegiatan/indikator/detail') }}"+'/'+id,
                dataType: "json",
        ...
        $('#create').click(function(){
            $('#sub_kegiatan_indikator_form')[0].reset();
        ...
        $('#sub_kegiatan_indikator_form').on('submit', function(e){
            e.preventDefault();
            if($('#aksi').val() == 'Save')
            {
                $.ajax({
                    url: "{{ url('/admin/sub-kegiatan/indikator') }}",
                    method: "POST",
                    data: $(this).serialize(),
                    dataType: "json",
                    beforeSend: function()
                    ...
                    success: function(data)
                    {
                        ...
                        if(data.errors)
                        {
                            ...
                            $('#sub_kegiatan_indikator_form')[0].reset();
                            $('#aksi_button').text('Save');
                            $('#sub_kegiatan_indikator_table').DataTable().ajax.reload();
                        }
                        if(data.success)
                        {
            ...
            {
                $.ajax({
                    url: "{{ url('/admin/sub-kegiatan/indikator/update') }}",
                    method: "POST",
                    data: $(this).serialize(),
                    dataType: "json",
                    beforeSend: function(){
        ...
        $(document).on('click', '.edit', function(){
            ...
            $('#form_result').html('');
            $.ajax({
                url: "{{ url('/admin/sub-kegiatan/indikator/edit') }}"+'/'+id,
                dataType: "json",
                success: function(data)
                {
        ...
        $(document).on('click', '.delete',function(){
            ...
            return new swal({
                ...
                {
                    $.ajax({
                        url: "{{ url('/admin/sub-kegiatan/indikator/destroy') }}" + '/' + id,
                        dataType: "json",
                        beforeSend: function()
                        {
                            return new swal({
