...
@section('title', 'Admin | Program | Indikator')
...
@php
    ...
    $cek_perubahan = PivotPerubahanProgram::where('program_id',$program->id)->latest()->first();
@endphp
<div class="container">
    ...
    <div class="page-title-container">
        ...
        <div class="col-12 col-md-7">
            <h1 class="mb-0 pb-0 display-4" id="title">Program</h1>
    ...
    <div class="card mb-5">
        <div class="card-body">
            @if ($cek_perubahan)
                <p>
                    Kode Urusan: {{$cek_perubahan->program->urusan->kode}} <br>
                    Urusan : {{$cek_perubahan->program->urusan->deskripsi}} <br>
                    <br>
                    Kode Program: {{$cek_perubahan->kode}}<br>
                    Program: {{$cek_perubahan->deskripsi}}<br>
                    Pagu: Rp. {{number_format($cek_perubahan->pagu, 2)}}<br>
                </p>
            @else
                <p>
                    Kode Urusan: {{$program->urusan->kode}} <br>
                    Urusan : {{$program->urusan->deskripsi}} <br>
                    <br>
                    Kode Program: {{$program->kode}}<br>
                    Program: {{$program->deskripsi}}<br>
                    Pagu: Rp. {{number_format($program->pagu, 2)}}<br>
                </p>
            @endif
        </div>
    </div>
    ...
    <div class="row mb-3">
        <div class="col-12" style="text-align: right">
            ...
            <a class="btn btn-outline-success waves-effect waves-light mr-2" href="{{ asset('template/template_impor_program_indikator.xlsx') }}" title="Download Template Import Data"><i class="fas fa-file-excel"></i></a>
    ...
    <div class="data-table-rows slim">
        ...
        <div class="data-table-responsive-wrapper">
            <table id="program_indikator_table" class="data-table w-100">
                <thead>
                    <tr>
    ...
    <div class="modal-dialog modal-dialog-centered modal-lg">
        <div class="modal-content">
            ...
            <div class="modal-body">
                ...
                <form id="program_indikator_form" class="tooltip-label-end" method="POST" novalidate enctype="multipart/form-data">
                    @csrf
...
<div id="importModal" class="modal fade" tabindex="-1" role="dialog" aria-labelledby="importModalLabel" aria-hidden="true">
    <div class="modal-dialog modal-dialog-centered modal-lg">
        <div class="modal-content">
            ...
            <div class="modal-body">
                <form action="{{ route('admin.program.indikator.impor') }}" class="form-horizontal" method="POST" enctype="multipart/form-data">
                    @csrf
                    <input type="hidden" name="program_id" value="{{$program->id}}">
                    <div class="mb-3 position-relative form-group">
                        <input type="file" class="dropify" id="impor_program_indikator" name="impor_program_indikator" data-height="150" data-allowed-file-extensions="xlsx" data-show-errors="true" required>
                    </div>
                    <div class="mb-3 position-relative form-group">
...
@section('js')
    ...
    <script>
        ...
        $(document).ready(function(){
            ...
            var dataTables = $('#program_indikator_table').DataTable({
                processing: true,
                serverSide: true,
                ajax: {
                    url: "{{ url('/admin/program/"+program_id+"/indikator') }}",
                    data: {
                        id : program_id
                    }
                },
        ...
        $(document).on('click', '.detail', function(){
            var id = $(this).attr('id');
            $.ajax({
                url: "{{ url('/admin/program/indikator/detail') }}"+'/'+id,
                dataType: "json",
        ...
        $('#create').click(function(){
            $('#program_indikator_form')[0].reset();
        ...
        $('#program_indikator_form').on('submit', function(e){
            e.preventDefault();
            if($('#aksi').val() == 'Save')
            {
                $.ajax({
                    url: "{{ url('/admin/program/indikator') }}",
                    method: "POST",
                    data: $(this).serialize(),
                    dataType: "json",
                    beforeSend: function()
                    ...
                    success: function(data)
                    {
                        ...
                        if(data.errors)
                        {
                            ...
                            $('#program_indikator_form')[0].reset();
                            $('#aksi_button').text('Save');
                            $('#program_indikator_table').DataTable().ajax.reload();
                        }
                        if(data.success)
                        {
            ...
            {
                $.ajax({
                    url: "{{ url('/admin/program/indikator/update') }}",
                    method: "POST",
                    data: $(this).serialize(),
                    dataType: "json",
                    beforeSend: function(){
        ...
        $(document).on('click', '.edit', function(){
            ...
            $('#form_result').html('');
            $.ajax({
                url: "{{ url('/admin/program/indikator/edit') }}"+'/'+id,
                dataType: "json",
                success: function(data)
                {
        ...
        $(document).on('click', '.delete',function(){
            ...
            return new swal({
                ...
                {
                    $.ajax({
                        url: "{{ url('/admin/program/indikator/destroy') }}" + '/' + id,
                        dataType: "json",
                        beforeSend: function()
                        {
                            return new swal({
