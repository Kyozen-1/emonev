...
<div class="container">
    ...
    <div class="page-title-container">
        ...
        <div class="col-12 col-md-7">
            <h1 class="mb-0 pb-0 display-4" id="title">Master TW</h1>
            <nav class="breadcrumb-container d-inline-block" aria-label="breadcrumb">
                <ul class="breadcrumb pt-0">
                    <li class="breadcrumb-item"><a href="{{ route('admin.dashboard.index') }}">Dashboard</a></li>
                    <li class="breadcrumb-item"><a href="#">Master Data</a></li>
                    <li class="breadcrumb-item"><a href="#">Master TW</a></li>
                </ul>
            </nav>
        </div>
        <!-- Title End -->
    ...
    <div class="data-table-rows slim">
        ...
        <div class="data-table-responsive-wrapper">
            <table id="master_tw_table" class="data-table nowrap w-100">
                <thead>
                    <tr>
    ...
    <div class="modal-dialog modal-dialog-centered modal-lg">
        <div class="modal-content">
            ...
            <div class="modal-body">
                ...
                <form id="master_tw_form" class="tooltip-label-end" method="POST" novalidate enctype="multipart/form-data">
                    @csrf
                    <div class="row">
                        <div class="mb-3">
...
@section('js')
    ...
    <script>
        $(document).ready(function(){
            ...
            var dataTables = $('#master_tw_table').DataTable({
                processing: true,
                serverSide: true,
                ajax: {
                    url: "{{ route('admin.master-tw.index') }}",
                },
                columns:[
                    {
                        data: 'DT_RowIndex'
        ...
        $(document).on('click', '.detail', function(){
            var id = $(this).attr('id');
            $.ajax({
                url: "{{ url('/admin/master-tw/detail') }}"+'/'+id,
                dataType: "json",
        ...
        $('#master_tw_form').on('submit', function(e){
            e.preventDefault();
            if($('#aksi').val() == 'Save')
            {
                $.ajax({
                    url: "{{ route('admin.master-tw.store') }}",
                    method: "POST",
                    data: $(this).serialize(),
                    dataType: "json",
                    beforeSend: function()
                    ...
                    success: function(data)
                    {
                        ...
                        if(data.errors)
                        {
                            ...
                            $('#master_tw_form')[0].reset();
                            $('#aksi_button').text('Save');
                            $('#master_tw_table').DataTable().ajax.reload();
                        }
                        if(data.success)
                        {
            ...
            {
                $.ajax({
                    url: "{{ route('admin.master-tw.update') }}",
                    method: "POST",
                    data: $(this).serialize(),
                    dataType: "json",
                    beforeSend: function(){
        ...
        $(document).on('click', '.edit', function(){
            ...
            $('#form_result').html('');
            $.ajax({
                url: "{{ url('/admin/master-tw/edit') }}"+'/'+id,
                dataType: "json",
                success: function(data)
                {
        ...
        $(document).on('click', '.delete',function(){
            ...
            return new swal({
                ...
                {
                    $.ajax({
                        url: "{{ url('/admin/master-tw/destroy') }}" + '/' + id,
                        dataType: "json",
                        beforeSend: function()
                        {
                            return new swal({
